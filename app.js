(() => {
  ...
  const W = 1100, H = 700, PLAYER_R = 15, BALL_R = 7;
  const STORAGE_KEY = 'implacables-rugby-tactics-v1';
  const TOOLS = [['select', 'Select'], ['erase', 'Erase'], ['run', 'Run'], ['carry', 'Carry'], ['pass', 'Pass'], ['kick', 'Kick'], ['text', 'Text'], ['highlight', 'Highlight']];
  const BOARD_VIEWS = [['pitchIn', 'Pitch In'], ['pitchMiddle', 'Pitch Middle'], ['pitchOut', 'Pitch Out'], ['lineout', 'Lineout']];
  ...
  const uid = (prefix = 'id') => `${prefix}-${crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`}`;
  const clone = value => JSON.parse(JSON.stringify(value));
  const clamp = (n, min, max) => Math.max(min, Math.min(max, n));
  const esc = text => String(text ?? '').replace(/[&<>"']/g, char => ({ '&':'&amp;', '<':'&lt;', '>':'&gt;', '"':'&quot;', "'":'&#39;' }[char]));

  const defaultStage = () => ({ boardView: 'pitchIn', players: [], ball: { x: 500, y: 460 }, actions: [] });
  const defaultData = () => ({ version: 2, metadata: { name: 'Untitled Play', description: '', tags: [] }, stages: [defaultStage()], currentStageIndex: 0 });
  ...
  function sanitize(value) {
    ...
    clean.metadata = { name: String(value.metadata?.name || 'Untitled Play').slice(0, 120), description: String(value.metadata?.description || '').slice(0, 1000), tags: Array.isArray(value.metadata?.tags) ? value.metadata.tags.slice(0, 12).map(x => String(x).slice(0, 32)) : [] };
    clean.stages = value.stages.slice(0, 120).map(stage => ({ boardView: BOARD_VIEWS.some(([id]) => id === stage.boardView) ? stage.boardView : 'pitchIn', players: Array.isArray(stage.players) ? stage.players.slice(0, 60).map(p => ({ id: String(p.id || uid('player')), team: p.team === 'defence' ? 'defence' : 'attack', number: String(p.number ?? ''), x: clamp(Number(p.x) || 0, 15, W - 15), y: clamp(Number(p.y) || 0, 15, H - 15) })) : [], ball: { x: clamp(Number(stage.ball?.x) || 500, 8, W - 8), y: clamp(Number(stage.ball?.y) || 460, 8, H - 8) }, actions: Array.isArray(stage.actions) ? stage.actions.slice(0, 160).map(a => ({ ...a, id: String(a.id || uid('action')) })) : [] }));
  ...
  function renderBoard(stage = activeStage()) {
    ...
    stage.players.forEach(player => renderPlayer(player));
    renderBall(stage.ball);
  }
  ...
  function applyPlaybackFrame(definition, amount) {
    ...
    const ball = interpolatePoint(definition.ball.from, definition.ball.to, amount);
  ...
  function startPlaybackTransition() {
    ...
    const definition = buildPlaybackDefinition(fromStage, toStage);
    const startStage = { ...fromStage, players: definition.players.map(player => ({ id: player.id, team: player.team, number: player.number, x: player.from.x, y: player.from.y })) };
    renderBoard(startStage);
    const animate = timestamp => {
      if (!playing || !playTransition) return;
      const amount = Math.min((timestamp - playTransition.startedAt) / playTransition.duration, 1);
      applyPlaybackFrame(definition, amount);
      if (amount >= 1) { completePlaybackTransition(); return; }
